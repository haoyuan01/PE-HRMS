...
export type MovementTypeFormValues = z.infer<typeof schema>;

interface MovementTypeFormModalProps {
  open: boolean;
  mode: "create" | "edit";
  defaultValues?: MovementTypeFormValues;
  isSaving?: boolean;
  onClose: () => void;
  onSubmit: (values: MovementTypeFormValues) => void;
}
...
export function MovementTypeFormModal({
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<MovementTypeFormValues>({
    resolver: zodResolver(schema),
    defaultValues: { name: "", description: "" },
  });

  // Sync the form with incoming values each time the modal opens so create
    ...
    >
      <div className="w-full max-w-md rounded-2xl bg-surface-container-lowest p-6 shadow-[var(--shadow-ambient)]">
        <h2 className="font-display text-lg font-bold text-on-surface">
          {mode === "create" ? "New Movement Type" : "Update Movement Type"}
        </h2>

        <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-5">
          {/* Name */}
          <div className="space-y-2">
            <Label htmlFor="movement-type-name" className={FIELD_LABEL}>
              Name *
            </Label>
            <Input
              id="movement-type-name"
              placeholder="Enter movement type name"
          ...
          {/* Description */}
          <div className="space-y-2">
            <Label htmlFor="movement-type-description" className={FIELD_LABEL}>
              Description
            </Label>
            <textarea
              id="movement-type-description"
              rows={4}
